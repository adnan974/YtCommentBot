export async function collectLinks(page) {
  // Récupérer les liens et les vues associées
  const videoData = await page.$$eval("ytd-video-renderer", (videos) => {
      return videos.map((video) => {
          ...
          const viewsMatch = viewsText.match(/([\d,.]+)\s?(K|M|B)?\s?views?/i);
      ...
      .filter((video) => video && video.link && video.views > 1000 && video.views < 10000);
  });
